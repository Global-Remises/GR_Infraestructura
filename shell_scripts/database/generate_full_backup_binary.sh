#!/bin/bash
#name: generate_full_backup_binary.sh
...
CMD="pg_dump --host $HOST --port $PORT --username $USERNAME --dbname $DBNAME --no-password --format=custom>> $OUTPUT"
eval $CMD